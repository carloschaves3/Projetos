...
decifra:
	...
	dec	eax
	mov	[len_texto_cifrado], eax

	; Le do arquivo para o vetor "texto_cifrado" de tamanho 20000 bytes
	mov	eax, 3
	mov	ebx, [ebp+8]
	mov	ecx, texto_cifrado
	mov	edx, 20000
	int	80h
	
	; Caso arquivo nao contenha nada
	mov	eax, texto_cifrado
	cmp	byte[eax], 0
	je	arquivo_vazio
	jmp	continua
arquivo_vazio:
...
continua:
	...
	; Antes de fazer qualquer coisa percorre os caracteres do vetor "texto_cifrado"
...
fim_laco_salva:
	...
	; Tirando espacos e enters do vetor "texto_cifrado" para juntar os caracteres em uma unica linha
	mov	eax, texto_cifrado
	dec	eax
	mov	ecx, -1
	mov	edx, 0
...
proxim:
	...
	je	proxim
	mov	dx, [eax]
	mov	[texto_cifrado+esi], dx
	inc	esi
	jmp	proxim
fim_vetor:
	mov	dword [len_texto_cifrado], esi
	inc	esi
	mov	byte [texto_cifrado+esi], 0
...
fim_loop_chave:
	...
	; Igualando tamanhos do "texto_cifrado" e da "chave"
	mov	eax, texto_cifrado
	mov	ebx, chave
	mov	ecx, [len_chave]
	mov	esi, -1
...
fim_tamanho:
	...
	; Passando "texto_cifrado" e "chave" tudo para maiusculo para usar a logica da cifra
	mov	eax, texto_cifrado
	mov	ebx, chave
	mov	esi, -1
	dec	eax
